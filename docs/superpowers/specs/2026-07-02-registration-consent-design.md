...
5. **No runtime brand/network variable interpolation in v1.** Consent content (`title`, `content` fields in each document version) is **pre-authored** with literal branding in each per-network or per-brand `consent.json` file. There is no `{{brand}}`, `{{network}}`, or any other template-variable substitution at runtime — the content is served exactly as authored. Authors must write the final branded text into the config file itself. Runtime interpolation is deferred to a future iteration.
