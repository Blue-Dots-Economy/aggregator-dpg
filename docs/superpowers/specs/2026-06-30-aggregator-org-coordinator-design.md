...
| `ensureSignalstackOrg`  | —   | ✅          | coordinator's own signalstack org. `domains` on the upsert = the coordinator's `type` (seeker/provider) when it matches an active-network domain, else all network domains — **status-quo upsert logic, unchanged**; the coordinator keeps its type (see §2) |
