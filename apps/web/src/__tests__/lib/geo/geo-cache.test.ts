/**
 * Unit tests for the geo lookup cache.
 *
 * Ported from Signals-DPG `apps/ui/src/lib/geo/geo-cache.test.ts` alongside the
 * implementation, so the two copies stay verifiably equivalent rather than
 * merely looking alike. The ref-counted abort is the subtle part: one caller
 * aborting (the user typing the next keystroke) must cancel a fetch nobody else
 * is waiting on, while never cancelling one a second caller still wants.
 */
import { describe, it, expect, vi } from 'vitest';
import { normalizeGeoKey, memoizeGeoLookup, withGeoCache } from '@/lib/geo/geo-cache';
