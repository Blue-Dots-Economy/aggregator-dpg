 ...
 *
 * Errors are returned in the canonical envelope:
 *   { error: { code, title, detail, requestId, timestamp } }
 *
 * Upstream API errors are forwarded verbatim — the API is the source of
 * truth for the envelope. This route only synthesises an envelope when the
 * upstream call could not be made at all (no service token, network
 * failure, malformed JSON request).
 *
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const log = logger.child({ reqId, route: 'POST /api/aggregator/register' });
  ...
  try {
    ...
    log.error(
      ...
      'aggregator registration upstream call failed',
    );
    return jsonError(
      503,
      envelope(
