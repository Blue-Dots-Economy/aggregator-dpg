...
`lua-loader.ts` loads the script once at module init and calls it via `EVALSHA` (fast path) — falling back to a full `EVAL` on a `NOSCRIPT` error (Redis restarted and flushed its script cache). The digest `EVALSHA` keys on is obtained from Redis with `SCRIPT LOAD` on first use and cached per client, rather than hashed locally: `EVALSHA` is SHA1-only by protocol, and computing SHA1 in application code trips the weak-hash rule (`typescript:S4790`). If you're changing `bulk_row_commit.lua`, remember the source is read from disk at process start — a running worker won't pick up a script edit until it restarts.
