 ...
 */
export const bulkUploadBatch = pgTable(
  'bulk_upload_batch',
  {
    id: uuid('id')
      .primaryKey()
  ...
  (t) => ({
    ...
    aggregatorCreatedIdx: index('idx_bulk_upload_batch_aggregator_created').on(
      t.aggregatorId,
      t.createdAt.desc(),
    ),
  }),
 ...
 */
export const bulkUploadRow = pgTable(
  'bulk_upload_row',
  {
    id: uuid('id')
      .primaryKey()
  ...
  (t) => ({
    /** Batch row listing sorted by row_number for findByBatch(). */
    batchRowIdx: index('idx_bulk_upload_row_batch_row').on(t.batchId, t.rowNumber.desc()),
  }),
);
