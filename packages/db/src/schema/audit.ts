...
export const auditLog = pgTable(
  ...
  (t) => ({
    /** Audit trail listing for findByAggregator(), sorted by event time newest-first. */
    aggregatorOccurredIdx: index('idx_audit_log_aggregator_occurred').on(
      t.aggregatorId,
      t.occurredAt.desc(),
    ),
    /** Entity-scoped audit trail for findByEntity(). */
    entityOccurredIdx: index('idx_audit_log_entity_occurred').on(
      t.entity,
      t.entityId,
      t.occurredAt.desc(),
    ),
