...
export class HttpSignalStackWriter extends SignalStackWriterBase {
  ...
  override async onboard(
    ...
    if (submitMode !== 'account_only') {
      ...
      // Caller-resolved coordinates suppress signals' own geocoding of the
      // address text in item_state. Only meaningful alongside an item, so it
      // rides inside the same branch — an `account_only` push creates no item
      // for the coordinates to belong to.
